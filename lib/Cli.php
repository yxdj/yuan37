...
{
	//解析参数
	public static function parseArgs($argv,$argc){
		$args=array();
		$null=false;
	...
	//停留模式
	public static function stop($info='enter key go on...',$default=false){
		while(true){
			fwrite(STDOUT,$info);
	...
	//选择模式
	public static function option($info='please input:',$options=array(),$default=false){
		while(true){
			fwrite(STDOUT,$info);
			$answer=trim(fgets(STDIN));
	...
	//书写模式
	public static function ask($info='please input:',$over=false){	
